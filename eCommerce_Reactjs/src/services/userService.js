...
const getProductRecommendService = (data) => {
    return axios.get(`/api/get-product-recommend?userId=${data.userId}&limit=${data.limit}&productId=${data.productId}`)
}
...
const getAllComment= (data) => {
    return axios.get(`/api/get-all-comment`, data)
}
const deleteCommentServiceById = (id) => {
    return axios.delete(`/api/delete-comment-by-commentId?id=${id}`)
...
export {
    ...
    getStatisticStockProduct, getExchangeRate, paymentOrderVnpayService, confirmOrderVnpay, paymentOrderVnpaySuccessService,getAllComment,deleteCommentServiceById
}