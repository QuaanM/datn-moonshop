            ...
            if (!data) {
                ...
                if (res && res.length > 0) {
                    ...
                    for (let i = 0; i < res.length; i++) {
                        ...
                        res[i].product = await db.Product.findOne({ where: { id: res[i].productId } })
                        res[i].blog = await db.Blog.findOne({ where: { id: res[i].blogId } })
                        res[i].user.image = new Buffer(res[i].user.image, 'base64').toString('binary')
                    }
                }
...
let deleteCommentByCommentId = (id) => {
    return new Promise(async (resolve, reject) => {
        try {
            if (!id) {
                resolve({
                ...
                let res = await db.Comment.findOne({
                    where: {
                        id: id
                    },
                    order: [['createdAt', 'DESC']],
